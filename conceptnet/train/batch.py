...
import conceptnet.train.train
from conceptnet.gpt import prepare_position_embeddings
...
def mle_steps(key, model, input_, targets, attention_mask,
    ...
    word_dist = conceptnet.train.train.modify_output_for_loss_fn(
        "nll", word_acts, dim=-1)
...
def update_generation_losses(losses, nums, micro, macro, bs,
                             length, loss, split):
    if split == "train":
        conceptnet.train.train.update_generation_losses(
