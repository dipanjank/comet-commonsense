...
from conceptnet.data import config as cfg
from conceptnet.data import data_utils
...
class GenerationDataLoader:
    ...
    def make_tensors(self, text_encoder, special,
        ...
        logging.info(self.max_e1)
        logging.info(self.max_r)
        logging.info(self.max_e2)
        ...
        for split in splits:
            ...
            for i, seq in enumerate(sequences[split]):
                # logging.info(self.sequences[split]["total"][i, :len(seq[0])].size())
                # logging.info(torch.FloatTensor(seq[0]).size())
    ...
    def shuffle_sequences(self, split="train", keys=None):
        if keys is None:
            ...
            # logging.info(type(self.data.keys()))
